...
public class QuadTree {
  ...
  public void retrieveFast(Array<Rectangle> list, Rectangle area) {
    ...
    if (level == MAX_LEVEL || level == MAX_LEVEL - 1 || level == MAX_LEVEL - 2)
      list.addAll(objects);
  }
  ...
  private void split() {
    ...
    nodes[3] =
        new QuadTree(level + 1, new Rectangle(x + subWidth, y + subHeight, subWidth, subHeight));
  }
  ...
  private int getIndex(Rectangle pRect) {
    ...
    boolean topQuadrant =
        pRect.getY() < horizontalMidpoint && pRect.getY() + pRect.getHeight() < horizontalMidpoint;
